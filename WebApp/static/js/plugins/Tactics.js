	...
	if (option.length > 0) {
		...
		for (k in data) {
			...
			if (format == "table") {
				...
				// {name : 'Buy', value : 20.61, xAxis: '2016-12-12', yAxis:
				// 20.61}
				var mpList = []
...
function QuantChart(data, k, format) {
...
function showTable(o) {
	...
	var table = "<table class='table table-striped'>" + "<thead>" + "<tr>" + th
			+ "</tr>" + "</thead>"
			+ "<tbody id='infoshowtd' name='infoshowtd'>" + td + "</tbody>"
			+ "</table>";
...
function addTables(table) {
	var codeHtml = "<li class='media'>"
			...
			+ "<a href='#'>站长</a></h4>" + table + "<hr>" + "</div>" + "</li>";
